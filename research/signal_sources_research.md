...
5. **币Coin（P2，依赖账号代理）**——bicoin.com.cn 没有官方开放 API；移动端使用 Native 层加密（sign+data，frida hook），逆向成本高；最稳妥做法和 参考行业惯例：让最终用户上传自己的币Coin 账号密码 / cookie，由我方代理登录后从已认证的私有接口拉取大佬持仓。法律与口碑风险较高，需用户明示授权 + 走加密保险柜。
...
落地建议：**第 1 期只做 Hyperliquid + EVM 聪明钱 + OKX public-copytrading**，三周内可端到端跑通；第 2 期再叠 Binance Leaderboard scraper + 币Coin 代理登录；隐藏持仓这一类"销售卖点"放到 SVIP 套餐里、走"用户自己上传 cookie"模式以转嫁合规风险（同类产品惯常做法）。
...
公共接口对 `positionShared=false` 的账户**完全无能为力**——这是隐藏持仓跟单的核心难点。可行的（合法性递减）方案：
...
3. **侧信道推断**：从公开聚合数据（资金费率、24h持仓量、Binance 大额异常成交、leaderboard 排名变化 + ROI 反推仓位）做"统计回填"——精度差、延迟高、误差大，同类产品鲜有采用。
